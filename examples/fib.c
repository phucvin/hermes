...
L0:
  ...
  frame[2] = _sh_ljs_get_string(shr, get_symbols(shUnit)[2] /*=======*/);
  frame[5] = _sh_ljs_undefined();
  frame[3] = _sh_ljs_undefined();
  locals.t1 = _sh_ljs_call(shr, frame, 1);
  frame[4] = _sh_ljs_try_get_by_id_rjs(shr,&locals.t0, get_symbols(shUnit)[1] /*print*/, get_prop_cache(shUnit) + 1);
  frame[5] = _sh_ljs_undefined();
  frame[3] = _sh_ljs_undefined();
  frame[2] = np1;
...
static SHNativeFuncInfo s_function_info_table[] = {
  ...
  { .name_index = 4, .arg_count = 1, .prohibit_invoke = 2, .kind = 0 },
};
...
static const uint32_t s_strings[] = {0,0,0,1,5,2794059355,7,7,3191741846,15,6,615793799,22,3,2473893398,};
#define CREATE_THIS_UNIT sh_export_this_unit
struct UnitData {
  SHUnit unit;
  SHSymbolID symbol_data[5];
  SHPropertyCacheEntry prop_cache_data[2];
